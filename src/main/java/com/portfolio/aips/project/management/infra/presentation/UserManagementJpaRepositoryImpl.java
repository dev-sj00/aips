...
public class UserManagementJpaRepositoryImpl implements UserManagementRepository {
    ...
    @Override
    public Page<FindAllUsersResultVO> findAllUsersWithOffsetAndLimit(int page, int size) {
        Pageable pageable = PageRequest.of(page, size);

        Page<UsersEntity> entities = usersRepository.findAll(pageable);

        return entities.map(user -> new FindAllUsersResultVO(
                user.getPk(),
                user.getNickname(),
    ...
    @Override
    @Transactional
    public void updateUserRoleByUserPk(Long userPk, UserRole role) {
    ...
    @Override
    public void createReportInProgressByAdminUserPkAndTargetUserPk(Long adminUserPk, Long targetUserPk) {
